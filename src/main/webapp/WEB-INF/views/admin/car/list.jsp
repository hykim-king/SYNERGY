	...
	<main>
		<div class="btn-main">
			...
			<a href="${pageContext.request.contextPath}/admin/car/add.do"
				style="margin-left: 10px; background-color: #27ae60;">차량 등록</a>
		</div>
		...
		<form class="search-form" method="get" action="list.do"
			...
			<select name="searchType">
				...
				<option value="cartype"
					<c:if test="${searchType == 'cartype'}">selected</c:if>>차종</option>
		...
		<table>
			...
			<tbody>
				<c:if test="${empty carList}">
					<tr>
						<td colspan="11" style="text-align: center;">등록된 차량 정보가 없습니다.</td>
					</tr>
				</c:if>
				<c:forEach var="car" items="${carList}" varStatus="status">
					<tr>
						...
						<td><a
							href="${pageContext.request.contextPath}/admin/car/delete.do?carCode=${car.carCode}"
							onclick="return confirm('정말 삭제하시겠습니까?');"
							class="action-btn delete">삭제</a> <a
							href="${pageContext.request.contextPath}/admin/car/updateView.do?carCode=${car.carCode}"
							class="action-btn">수정</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
